...
uses
  System.SysUtils, System.Classes, Vcl.Controls, Vcl.StdCtrls, Vcl.ExtCtrls, Objekt.EditAutoCompleteList,
  Objekt.EditAutoComplete;
...
type
  ...
  public
    ...
    function SelectedItem: TEditAutoComplete;
...
procedure Register;
begin
  RegisterComponents('Optima', [TnfsEditAutoComplete]);
end;
...
begin
  ...
  if Key <> 13 then
    exit;
  Text := _ListBox.Items[_ListBox.ItemIndex];
  fSelectedItem := TEditAutoComplete(_ListBox.Items.Objects[_ListBox.ItemIndex]);
  _ListBox.Visible := false;
  SetFocus;
  SelStart := Length(Text);
end;
...
function TnfsEditAutoComplete.SearchList: TEditAutoCompleteList;
begin
  Result := fSearchList;
end;

function TnfsEditAutoComplete.SelectedItem: TEditAutoComplete;
begin
