...
uses
  ...
  MidasLib, o_lpUser, o_lpUserList, o_LockRecievedMsg;
...
type
  ...
  TLogMessageEvent = procedure(Sender: TObject; aText: string; aMessageTyp : TMsgDlgType) of object;
...
type
  ...
  public
    ...
    AppType: TAppType;
    Lastupdate: TDateTime;
    function getRecievedMsg: string;
  end;
...
type
  ...
  private
    ...
    procedure Login(Mandant: integer; aMandantName, User, Machine, Session: string; aErzwingeAnmeldung: string; AppType: TAppType; Socket: TCustomWinSocket);
    procedure Logout(aMandant: integer; aUser, aSession, aMachine: string; aSocket: TCustomWinSocket);
    ...
    procedure setConnection(const Value: TServerSocket);
  protected
  public
    fLockRecievedMsg: TLockRecievedMsg;
  ...
  published
    property ServerSocket: TServerSocket read FConnection write setConnection;
...
type
  ...
  private
    ...
    FOnLogMessageEvent: TLogMessageEvent;
    ...
    function SendAndGetMsg(MessageText: String; const aOnlySend: Boolean = false): string;
    function getMachineName: string;
    function getActiveSession: string;
    function getCurrentUserLimit: string;
    procedure NachrichtVomServer(Sender: TObject; Socket: TCustomWinSocket);
    procedure setActiveSession(const Value: string);
    procedure setConnection(const Value: TClientSocket);
  ...
  public
    ...
    /// </para>
    /// </returns>
    function Login(User, Machine, Session: string;  aErzwingeAnmeldung: Boolean; AppType: TAppType; aMitarbeitername: string): integer;
    function VerbrauchteLizenzen: Integer;
    function AnzahlLizenzen: Integer;
    function LizenzUser: string;
    function BereitsAngemeldet(aUser, aMandant, aMachine: string): Boolean;

    /// <summary>
    /// Von VeriLock abmelden. Entfernt den Benutzer und die Lizenz
    /// </summary>
    procedure Logout(User, Machine, Session: string); overload;
    procedure Logout(User, Machine, Session, MandantId: string); overload;
    ...
    function GetLogListAll: string;
    function IsLoggedIn(User: string): boolean;
    procedure KillUser(User, Machine, Session: string);
    procedure Alive(User, Machine, Session: string);
    function SessionExist(User, Machine, Session: string): Boolean;
    ...
    procedure Prepare(aMandantID: integer; aMandantName: string);
    function CheckConnection(var aErrMsg : string): boolean;
    function BefehlExist(aBefehl: string): Boolean;
    property MandantId: Integer read fMandantId;
    property MandantName: string read fMandantname;
  published
    property ClientSocket: TClientSocket read FConnection write setConnection;
    property CurrentUserLimit : string read getCurrentUserLimit;
    property OnLogMessageEvent : TLogMessageEvent read FOnLogMessageEvent write FOnLogMessageEvent;
  end;
...
begin
  inherited;
  fLockRecievedMsg := TLockRecievedMsg.Create;
...
var
  ...
  IncID, Mandant, Machine, Key, ID, User, Hint, AppType, Session, Erzwingen, Mandantname: string;
  bCheckTimer: Boolean;
begin
  bCheckTimer := CheckTimer.Enabled;
  try
    CheckTimer.Enabled := false;
    {
    while fInProgress do
    begin
  ...
  //    FOnSocketInterceptRead(Sender, Socket, RecievedMessage);
   ...
   //        FOnSocketInterceptRead(Sender, Socket, Logtext);
      ...
      begin
        ...
        Delete(RecievedMessage, 1, pos(' ', RecievedMessage));
        fLockRecievedMsg.Msg := RecievedMessage;
        Mandant := fLockRecievedMsg.getItem(0);
        User := fLockRecievedMsg.getItem(1);
        Machine := fLockRecievedMsg.getItem(2);
        Session := fLockRecievedMsg.getItem(3);
        Erzwingen := fLockRecievedMsg.getItem(4);
        AppType := fLockRecievedMsg.getItem(5);
        Mandantname := fLockRecievedMsg.getItem(6);

        if AppType = 'S' then
          Login(StrToInt(Mandant), MandantName, User, Machine, Session, Erzwingen, AppTypeServer, Socket)
        else
          Login(StrToInt(Mandant), MandantName, User, Machine, Session, Erzwingen, AppTypeClient, Socket);
      end
      ...
      begin
        ...
        Logout(StrToInt(Mandant), User, Session, Machine, Socket);
  ...
  //         FOnSocketInterceptRead(Sender, Socket, Logtext);
    ...
    fInProgress := false;
    CheckTimer.Enabled := bCheckTimer;
  end;
...
procedure TLockProvider.Login(Mandant: integer; aMandantName, User, Machine, Session, aErzwingeAnmeldung: string;
...
//        Write(self, Socket, 'LOGIN ERR LIMIT Client limit exceedet');
    ...
    NewLogin.MandantName := aMandantName;
    NewLogin.User := User;
...
procedure TLockProvider.Logout(aMandant: integer; aUser, aSession, aMachine: string; aSocket: TCustomWinSocket);
var
  x: integer;
  fLoggedOut: Boolean;
  User: TUserItem;
begin
  ...
  try
    try
      ...
      while (x <= LoggedUserList.Count - 1) do
      begin
        User := TUserItem(LoggedUserList[x]);
        if (User.User = aUser) and
           (User.Mandant = aMandant) and
           (User.Session = aSession) then
        begin
          LoggedUserList.Delete(x);
          fLoggedOut := true;
          if Assigned(FOnLogUpdate) then
            FOnLogUpdate(self, 'Benutzer ' + User.User + ' wurde abgemeldet.');
          break;
        end;
      ...
      LoggedUserList.Pack;
      LoggedUserList.Capacity := LoggedUserList.Count;
      Write(self, aSocket, 'LOGOUT OK');
    except
      Write(self, aSocket, 'LOGOUT OK');
    end;
  finally
    if not fLoggedOut then
      FOnLogUpdate(self, 'Benutzer ' + aUser + ' konnte nicht abgemeldet werden.');
  end;
end;
...
begin
  try
    ...
    begin
      ...
      begin
        MessageStr := MessageStr + (LoggedUserList[x] as TUserItem).getRecievedMsg;
        {
        MessageStr := MessageStr + '(';
...
begin
  try
    ...
    begin
      ...
      MessageStr := MessageStr + (LoggedUserList[x] as TUserItem).Session + '|';
      MessageStr := MessageStr + (LoggedUserList[x] as TUserItem).Machine + '|';
      MessageStr := MessageStr + IntToStr((LoggedUserList[x] as TUserItem).Mandant) + '|';
      MessageStr := MessageStr + (LoggedUserList[x] as TUserItem).Mandantname + '|';
      MessageStr := MessageStr + DateTimeToStr((LoggedUserList[x] as TUserItem).Lastupdate) + ')'+slinebreak;
      }
      inc(x);
    end;
...
begin
  ...
  begin
    ...
    FConnection.OnRead := NachrichtVomServer;
...
procedure TLockProviderClient.NachrichtVomServer(Sender: TObject;
  Socket: TCustomWinSocket);
var
  s: string;
begin
  s := String(Socket.ReceiveText);
  if s = 'LebstDuNoch?' then
    Socket.SendText(#1+'JaIchLebeNoch'+#2);
end;
...
begin
  ...
  begin
     ShowMessage('Verilock scheint nicht zu laufen.');
  end;
  ErrorCode := 0;
end;
...
begin
  ...
  try
    ReceivedMessage := SendAndGetMsg('ADDLOCK ' + inttostr(fMandantID) + '|' +
...
begin
  ...
  try
    ...
    ReceivedMessage := SendAndGetMsg('ADDLOCKID ' + inttostr(fMandantID) + '|' +  inttostr(IncID) + '|' +
            Machine + '|' + inttostr(Key) + '|' + inttostr(ID) + '|' + User + '|' + Session + '|' + Hint);
...
begin
  Init(true);
  ReceivedMessage := SendAndGetMsg('DELLOCK ' + inttostr(fMandantID) + '|' + inttostr(LockID));
  ...
  begin
    ...
    if Assigned(FOnLogMessageEvent) then
    begin
      FOnLogMessageEvent(Self,'response mismatch. could not get data. LockId =' + inttostr(LockID), mtError);
    end;
//    ShowMsg(nil, 'response mismatch. could not get data. LockId =' + inttostr(LockID), mtError, [mbOk]);
    //raise Exception.Create('response mismatch. could not get data. LockId =' + inttostr(LockID));
    exit;
  end;

end;

function TLockProviderClient.GetLockList2: string;
begin
  Result := '';
  Init(true);

  ReceivedMessage := SendAndGetMsg('GETLOCKLIST ' + inttostr(fMandantID));
...
begin
  ...
  ReceivedMessage := SendAndGetMsg('ISTABLELOCKED ' + inttostr(fMandantID) + '|' + inttostr(Key));
...
function TLockProviderClient.Login(User, Machine, Session: string; aErzwingeAnmeldung: Boolean; AppType: TAppType; aMitarbeitername: string): integer;
var
  erzwingen : string;
begin
  erzwingen := 'F';
  ...
  if AppType = AppTypeServer then
    ReceivedMessage := SendAndGetMsg('LOGIN '+ inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session + '|' + erzwingen +'|S|'+ fMandantName+'|'+aMitarbeitername)
  else
    ReceivedMessage := SendAndGetMsg('LOGIN '+ inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session + '|' + erzwingen +'|C|'+ fMandantName+'|'+aMitarbeitername);
...
function TLockProviderClient.VerbrauchteLizenzen: Integer;
begin
  Result := 0;
  ReceivedMessage := SendAndGetMsg('GETANZVERBRLIZENZEN');
  if not TryStrToInt(ReceivedMessage, Result) then
    Result := 1;
end;


function TLockProviderClient.AnzahlLizenzen: Integer;
begin
  Result := 0;
  ReceivedMessage := SendAndGetMsg('GETANZAHLLIZENZEN');
  if not TryStrToInt(ReceivedMessage, Result) then
    Result := 1;
end;

function TLockProviderClient.BereitsAngemeldet(aUser, aMandant,
  aMachine: string): Boolean;
begin
  ReceivedMessage := SendAndGetMsg('BEREITSANGEMELDET ' + aUser + '|' + aMandant + '|' + aMachine);
  Result := ReceivedMessage = '1';
end;

function TLockProviderClient.LizenzUser: string;
begin
  Result := SendAndGetMsg('GETLIZENZUSER');
end;



procedure TLockProviderClient.Logout(User, Machine, Session, MandantId: string);
begin
  Init(true);

  ReceivedMessage := SendAndGetMsg('LOGOUT ' + MandantId + '|' + User + '|' + Machine + '|' + Session);
...
begin
  ...
  ReceivedMessage := SendAndGetMsg('LOGOUT ' + inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session);
...
var
  UserList: Tlp_UserList;
  i1: Integer;
begin

  Init(true);

  ReceivedMessage := SendAndGetMsg('GETUSERLIST ' + inttostr(fMandantID));
  ...
  UserList := Tlp_UserList.Create;
  try
    UserList.setReceivedMsg(ReceivedMessage);
    for i1 := 0 to UserList.Count -1 do
    begin
      if UserList.Item[i1].Mandant <> inttostr(fMandantID) then
        continue;
      Users.Append;
      Users.FieldByName('User').AsString := UserList.Item[i1].User;
      Users.FieldByName('Session').AsString := UserList.Item[i1].Session;
      Users.FieldByName('Machine').AsString := UserList.Item[i1].Machine;
      Users.FieldByName('LastUpdate').AsString := UserList.Item[i1].LastUpdate;
      Users.Post;
    end;
  finally
    FreeAndNil(UserList);
...
begin
  ...
  aUserList.setReceivedMsg(ReceivedMessage);

end;


function TLockProviderClient.GetLogListAll: string;
begin
  Result := SendAndGetMsg('GETLOGLISTALL');
end;
...
begin
  ...
  ReceivedMessage := SendAndGetMsg('ISLOGGEDIN ' + inttostr(fMandantID) + '|' + User);
...
begin
   ...
   Init(true);
  ReceivedMessage := SendAndGetMsg('KILLUSER ' + inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session);
...
procedure TLockProviderClient.Alive(User, Machine, Session: string);
begin
   Init(true);
  ReceivedMessage := SendAndGetMsg('ALIVE ' + inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session, true);
end;

function TLockProviderClient.SessionExist(User, Machine, Session: string): Boolean;
begin
   Init(true);
  ReceivedMessage := SendAndGetMsg('SESSIONEXIST ' + inttostr(fMandantID) + '|' + User + '|' + Machine + '|' + Session);
  Result := ReceivedMessage = '1';
end;



procedure TLockProviderClient.setActiveSession(const Value: string);
begin
  CurrentActiveSession := Value;
end;

procedure TLockProviderClient.setConnection(const Value: TClientSocket);
begin
  FConnection := Value;
end;
...
procedure TLockProviderClient.Prepare(aMandantID: integer; aMandantName: string);
begin
  fMandantID := aMandantID;
  fMandantName := aMandantName;
end;

// SendAndGetMsg ---------------------------------------------------------------
function TLockProviderClient.SendAndGetMsg(MessageText: String; const aOnlySend: Boolean = false): string;
var
  Timeout: boolean;
...
begin
  ...
  begin
    ...
    begin
      if Assigned(FOnLogMessageEvent) then
      begin
        FOnLogMessageEvent(Self,'request timed out: ' + MessageText, mtError);
      end;
      //ShowMsg(Self, 'request timed out: ' + MessageText, mtError, [mbOk]);
      //raise Exception.Create('request timed out: ' + MessageText);
    end;
  end;
...
procedure TLockProvider.setConnection(const Value: TServerSocket);
begin
  FConnection := Value;
end;
...
function TLockProviderClient.BefehlExist(aBefehl: string): Boolean;
var
  BefehlsList: TStringList;
  i1: Integer;
begin
  Result := false;
  BefehlsList := TStringList.Create;
  try
    BefehlsList.Text :=  SendAndGetMsg('GETBEFEHLSLIST');
    for i1 := 0 to BefehlsList.count -1 do
    begin
      if SameText(aBefehl, BefehlsList.strings[i1]) then
      begin
        Result := true;
        exit;
      end;
...
function TUserItem.getRecievedMsg: string;
begin
  Result := '(';
  Result := Result + User + '|';
  Result := Result + Session + '|';
  Result := Result + Machine + '|';
  Result := Result + IntToStr(Mandant) + '|';
  Result := Result + Mandantname + '|';
  Result := Result + DateTimeToStr(Lastupdate) + ')'+slinebreak;
end;
