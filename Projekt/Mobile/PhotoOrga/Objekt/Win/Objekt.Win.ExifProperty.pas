unit Objekt.Win.ExifProperty;

interface

uses
  System.SysUtils, System.Types,  System.Classes, System.Variants, Objekt.ExifPropertyTag, CloudExif;
...
{ TExifProperty }
 ...
 uses
    System.TypInfo, vcl.Imaging.jpeg, fmx.DialogService;


constructor TExifProperty.Create;
begin
  fExifImage := TAdvCloudExifImage.Create(nil);
...
begin
  ...
  try
    try
      ...
      fTag.ImageLength := Jpg.Height.ToString;
      fTag.ImageWidth  := Jpg.Width.ToString;
    except
      on E: Exception do
      begin
  ...
  if AFileName = 'D:\Bachmann\Daten\OneDrive\Bilder\Eigene Aufnahmen\IMG_20180724_170622.jpg' then
    x := 'dfD';
  if x = 'Hallo' then
    exit;

  try
    fExifImage.FileName := AFileName;   <-- Fehler hier drin
    if fExifImage.Orientation = TExifOrientation.eoTopRowLeftColumn then
      fOrientation := TEXIFOrientations.Normal;
    if fExifImage.Orientation = TExifOrientation.eoTopRowRightColumn then
      fOrientation := TEXIFOrientations.FlipHorizontal;
    if fExifImage.Orientation = TExifOrientation.eoBottomRowRightColumn then
      fOrientation := TEXIFOrientations.Rotate180;
    if fExifImage.Orientation = TExifOrientation.eoBottomRowLeftColumn then
      fOrientation := TEXIFOrientations.FlipVertical;
    if fExifImage.Orientation = TExifOrientation.eoLeftColumnTopRow then
      fOrientation := TEXIFOrientations.Transpose;
    if fExifImage.Orientation = TExifOrientation.eoRightColumnTopRow then
      fOrientation := TEXIFOrientations.Rotate90;
    if fExifImage.Orientation = TExifOrientation.eoRightColumnBottomRow then
      fOrientation := TEXIFOrientations.Rotate270;
    if fExifImage.Orientation = TExifOrientation.eoLeftColumnBottomRow then
      fOrientation := TEXIFOrientations.Rotate270;

    fOrientationStr := GetEnumName(TypeInfo(TEXIFOrientations), Ord(fOrientation));

    fLongitude := fExifImage.Longitude.ToString;
    fLatitude  := fExifImage.Latitude.ToString;
  except
    on E: Exception do
    begin
      TDialogService.ShowMessage(E.Message);
